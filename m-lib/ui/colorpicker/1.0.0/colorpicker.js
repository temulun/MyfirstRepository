/**
 * ColorPicker - Based on <https://github.com/PitPik/tinyColorPicker#1.1.0>
 * For docs see: <https://github.com/PitPik/tinyColorPicker/README.md>
 *
...
(function (root, factory) {
    ...
    // Colors {{{
    var Colors = (function(window, undefined) {
      "use strict"
    ...
    // @author Allex Wang
    function calculateZindex(el) {
        var $el, zIndex, rNum = /\d/, body = document.body;
        do {
            $el = $(el)
            if ( $el.css('position') !== 'static' && rNum.test(zIndex = $el.css('zIndex')) ) {
              return (parseInt(zIndex) || -1) + 1;
            }
            if (el === body) break;
     ...
     * @constructor
     */
    var colorPicker = function(seletor, options) {
        var $element = $(seletor);
        ...
        _instance = _instance.add($element);
        $element.colorPicker = _colorPicker || new ColorPicker(options);
        ...
        $element.on('focusin.tcp click.tcp', function(event) {
            _colorPicker.color.options = // swap options to fake new instance
                $.extend(_colorPicker.color.options, _options = $element.options);
            toggle.call(this, event);
        })
        .on('change.tcp', function() {
            _color.setColor(this.value || '#FFF');
            $element.colorPicker.render(true);
        })
        .each(function() {
    ...
    colorPicker.prototype.hide = function() {
      toggle.call(this.$element);
    }

    colorPicker.prototype.destroy = function() {
      this.$element.off('.tcp');
      this.$element = null;
    }
